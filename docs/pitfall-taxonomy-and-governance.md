...
**因此补强方向**：① 修 issue 固化回归测试（上文硬规则）；② 差分模糊测试
（随机脚本对 WSL GNU 5.3.0 逐字节比对，自动发现 83 套件外的分歧）；
③ 接入 POSIX test suite 与真实大脚本（bash-completion 等）作为第三口径；
④ 第四节数的"套件缺口"（解析器族无专属覆盖）逐步补齐。
